...
namespace Glory
{
	class GlrWorker
	{
		...
		private WeakReference<GlrTableParser> _outerWeak;
		GlrTableParser _Outer {
			get {
				if (null == _outerWeak)
					return null;
				GlrTableParser result;
				if (_outerWeak.TryGetTarget(out result))
					return result;
				return null;
			}
			set {
				_outerWeak = new WeakReference<GlrTableParser>(value);
			}
		}
		...
		public bool Read()
		{
			...
			if (0 < _stack.Count)
			{
				...
				if (0 == _tupleIndex)
				{
					...
					{
						_workers.Add(new GlrWorker(_Outer, this, k));
					}
				}
				if (null == trns)
				{
		...
		void _ReadNextToken()
		{
			...
			if(_tokenEnum.TryPeek(Index,out tok))
			{
				...
				{
					CurrentToken.Symbol = "#ERROR";
					CurrentToken.SymbolId = _errorId;
				}
				else
					CurrentToken.Symbol = _Outer.Symbols[CurrentToken.SymbolId];
				++Index;
			}
			else
			{
				CurrentToken.Symbol = "#EOS";
