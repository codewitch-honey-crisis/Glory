...
namespace GloryDemo {
    ...
    internal class JsonParser : GlrTableParser {
        ...
        public JsonParser(IEnumerable<Token> tokenizer) : 
                base(JsonParser.ParseTable, JsonParser.SymbolTable, JsonParser.ParseAttributes, JsonParser.ErrorSentinels, tokenizer, int.MaxValue) {
        }
        public JsonParser(IEnumerable<Token> tokenizer, int maxErrorCount) : 
                base(JsonParser.ParseTable, JsonParser.SymbolTable, JsonParser.ParseAttributes, JsonParser.ErrorSentinels, tokenizer, maxErrorCount) {
        }
        ...
        public const int fieldsList = 8;
        public const int valuesList = 9;
        ...
        public const int EosSymbol = 22;
        public const int ErrorSymbol = 23;
    }
}
namespace GloryDemo {
    using System;
    ...
    internal class GlrTableParser {
        ...
        public GlrTableParser(int[][][][] parseTable, string[] symbolTable, ParseAttribute[][] attributes, int[] errorSentinels, IEnumerable<Token> tokenizer) : 
                this(parseTable, symbolTable, attributes, errorSentinels, tokenizer, int.MaxValue) {
        }
        public GlrTableParser(int[][][][] parseTable, string[] symbolTable, ParseAttribute[][] attributes, int[] errorSentinels, IEnumerable<Token> tokenizer, int maxErrorCount) {
            ...
            if (this._tokenEnum.MoveNext()) {
                this._workers.Add(new GlrWorker(this, this.NextWorkerId, this._parseTable, this._errorId, this._eosId, this._errorSentinels, this._workers, this._tokenEnum));
        ...
        public string[] RuleDefinition {
            get {
                if ((false 
                            == (LRNodeType.Reduce == this._worker.NodeType))) {
                    return null;
                }
    ...
    internal class GlrWorker {
        ...
        GlrTableParser _Outer {
            get {
                if ((null == this._outerWeak)) {
                    return null;
                }
                GlrTableParser result;
                if (this._outerWeak.TryGetTarget(out result)) {
                    return result;
                }
                return null;
            }
            set {
                this._outerWeak = new WeakReference<GlrTableParser>(value);
            }
        }
        ...
        public bool Read() {
            ...
            if ((0 < this._stack.Count)) {
                ...
                if ((0 == this._tupleIndex)) {
                    for (int k = 1; (k < tbl.Length); k = (k + 1)) {
                        this._workers.Add(new GlrWorker(this._Outer, this, k));
                    }
                }
                if ((null == trns)) {
                    this._Panic();
                ...
                if ((1 == trns.Length)) {
                    ...
                    for (int j = 2; (j < trns.Length); j = (j + 1)) {
    ...
    internal partial class ParseNode {
        ...
        public bool HasErrors {
            get {
                if (("#ERROR" == this._symbol)) {
                    return true;
                }
                if (this.IsNonTerminal) {
                    for (int i = 0; (i < this.Children.Length); i = (i + 1)) {
                        if (this.Children[i].HasErrors) {
                            return true;
                        }
                    }
                }
